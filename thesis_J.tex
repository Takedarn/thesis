...
\toprule
\textbf{タスク} & \textbf{入力例} & \textbf{ラベル} \\
\midrule
\textbf{CoLA} & ``He go to the store.'' & unacceptable \\
\addlinespace
\textbf{SST-2} & ``The movie was fantastic and deeply moving.'' & positive  \\
\addlinespace
\textbf{MRPC} & \begin{tabular}[t]{@{}l@{}}``The company reported profits.''\\ ``The firm announced earnings.''\end{tabular} & equivalent \\
\addlinespace
\textbf{STS-B} & \begin{tabular}[t]{@{}l@{}}``A man is playing a guitar.''\\ ``Someone plays a stringed instrument.''\end{tabular} & 4.5 \\
\addlinespace
\textbf{QQP} & \begin{tabular}[t]{@{}l@{}}``How to bake a cake?''\\ ``What is the process to make a cake?''\end{tabular} & duplicate \\
\addlinespace
\textbf{MNLI} & \begin{tabular}[t]{@{}l@{}}前提: ``A dog is sleeping on the couch.''\\ 仮説: ``An animal is resting on furniture.''\end{tabular} & entailment \\
\addlinespace
\textbf{QNLI} & \begin{tabular}[t]{@{}l@{}}質問: ``Who wrote Hamlet?''\\ 文: ``Hamlet was written by Shakespeare.''\end{tabular} & relevant / entailment \\
\addlinespace
\textbf{RTE} & \begin{tabular}[t]{@{}l@{}}前提: ``The sky is clear and blue.''\\ 仮説: ``The sky has no color.''\end{tabular} & not-entailed \\
\bottomrule
\end{tabularx}
\end{table}

また、本研究においてはTinyBERTの公式設定に従い、各タスクに関してデータ拡張を用いる。学習においては拡張されたデータを用いるが、一部のタスクに関して本提案の適用により計算コストが増大するため、学習データのランダムサンプリングにより拡張データから一定割合のデータを選択して用いる。各タスクにおける拡張データの利用状況を表\ref{tab:data_augmentation_ratio}に示す。
...
\begin{tabular}{lrrr}
\toprule
& \textbf{オリジナル学習データ量} & \textbf{ランダムサンプリングの有無} & \textbf{学習データ量} \\
\midrule
\textbf{MRPC} & 3,669 & 無 & 225,098 \\
\textbf{CoLA} & 8,552 & 無 & 211,059 \\
\textbf{STS-B} & 5,750 & 無 & 320,113 \\
\textbf{RTE} & 2,491 & 無 & 68,796 \\
\textbf{QQP} & 363,847 & 有 & 1,221,763  \\
\textbf{MNLI} & 392,703 & 有 & 1,091,538 \\
\textbf{QNLI} & 104,744 & 無 & 1,250,571 \\
\textbf{SST-2} & 67,350 & 無 & 1,107,510 \\
\bottomrule
\end{tabular}
\end{table}

\subsection{提案手法の組み込みと学習}
実験では、公式のTinyBERTの実装をベースに提案手法である蒸留項を導入し、蒸留をすることでTinyBERTの精度を維持しつつ、教師モデルを同じ単語に着目して推論を行うことを目指す。TinyBERTの学習は2段階で行われるが、第1段階で隠れ層を教師モデルを模倣したモデルを用意し、第2段階で出力層を教師から模倣する段階に介入する。
